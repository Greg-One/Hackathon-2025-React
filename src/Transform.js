...
const Transform = ({ history, onLawClick, onStepNext }) => {
  ...
  const transform = useCallback((currentTag = '', currentNum = 0) => {
    const tagArray = (currentTag) || tag;

    const promises = tagArray.keywords.map((i) => fetch(encodeURI(`https://www.buymebuyme.xyz/?q=${i}`)));
        ...
        <section className="transform">
          <div className="transform__poem-container">
            ...
            <div className="transform__tag-group">
              ...
              <button
                type="button"
                className={classNames('button__translate', {
                  button__translate_disabled: !isTranslate,
                })}
